        ...
        if (!isNaN(Number(chainId))) {
            fs.readdirSync(path.join(tokensDir, chainId)).forEach(address => {
                ...
                if (type === "tokens" && tokenData.logoURI) {
                    ...
                    fs.mkdirSync(path.join(__dirname, "../src/images/tokens", chainId), { recursive: true });
                    fs.copyFileSync(
                        path.join(tokensDir, chainId, address, `logo.${logoExtension}`),
                        path.join(path.join(__dirname, "../src/images/tokens"), chainId, `${address.toLowerCase()}.${logoExtension}`)
                    )
                }
            })
        }
